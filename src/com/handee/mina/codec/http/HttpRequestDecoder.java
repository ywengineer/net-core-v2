...
public class HttpRequestDecoder extends MessageDecoderAdapter {
    ...
    public MessageDecoderResult decode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {
        // Try to decode body
        HttpRequestMessage m = decodeBody(session, in);
    ...
    private boolean messageComplete(IoBuffer in) throws Exception {
        ...
        if (in.get(0) == (byte) 'G' && in.get(1) == (byte) 'E' && in.get(2) == (byte) 'T') {
            ...
            // 0x0D 0x0A
            return (in.get(last) == (byte) 0x0A && in.get(last - 1) == (byte) 0x0D && in.get(last - 2) == (byte) 0x0A && in.get(last - 3) == (byte) 0x0D);
        } else if (in.get(0) == (byte) 'P' && in.get(1) == (byte) 'O' && in.get(2) == (byte) 'S' && in.get(3) == (byte) 'T') {
            // Http POST request,头文件结束标记
            ...
            for (int i = 0; i < last; i++) {
                ...
                for (int j = 0; j < CONTENT_LENGTH.length; j++) {
                    // System.out.println((char) in.get(i + j));
                    if (in.get(i + j) != CONTENT_LENGTH[j]) {
                        found = false;
                        break;
                    }
                    if (j == CONTENT_LENGTH.length - 1) {
                        found = true;
                    }
                }
                if (found) {
    ...
    private HttpRequestMessage decodeBody(IoSession session, IoBuffer in) {
        request = new HttpRequestMessage(session.getId());
        try {
            InetSocketAddress remoteAddress = (InetSocketAddress) session.getRemoteAddress();
            request.setHeaders(parseRequest(new StringReader(in.getString(decoder))));
            request.setRemoteAddress(remoteAddress.getAddress().getHostAddress());
            request.setRemotePort(remoteAddress.getPort());
            return request;
        } catch (CharacterCodingException ex) {
            // ex.printStackTrace();
            logger.error("decode http message body error :: ", ex);
        }
        ...
        try {
            ...
            if (url[0].equalsIgnoreCase("POST")) {
                ...
                if (position == len) {
                    ...
                    // 存在中文字符.
                    char[] newdata = new char[position];
                    // 将无效字符剔除
                    System.arraycopy(buf, 0, newdata, 0, position);
                    // 构建字符串
                    line = String.copyValueOf(newdata);
                }
            ...
            if (line != null) {
                ...
                for (String character : match) {
                    String[] params = new String[1];
                    String[] tokens = character.split("=");
                    switch (tokens.length) {
                        case 0:
                            map.put("@".concat(character), new String[]{});
                            break;
                        case 1:
