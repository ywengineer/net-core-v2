...
import com.handee.rpc.Connection;
import com.handee.rpc.FrameworkMessage;
import com.handee.rpc.Listener;
 ...
 * connections that have been {@link #addConnection(com.handee.rpc.Connection) added} are
 * allowed to {@link #getRemoteObject(com.handee.rpc.Connection, int, Class) access} registered
 * objects.
 * <p/>
 ...
 * {@link com.handee.rpc.rmi.RemoteObject#setNonBlocking(boolean) ignored}, an extra byte is
...
public class ObjectSpace {
			...
			if (connections != null) {
				...
				for (; i < n; i++)
					if (connection == connections[i])
						break;
				if (i == n)
					return; // The InvokeMethod message is not for a connection
							// in this ObjectSpace.
			}
	 ...
	 * {@link #addConnection(com.handee.rpc.Connection) added} to allow the remote end of the
	...
	/**
	 * Creates an ObjectSpace with the specified connection. More connections
	 * can be {@link #addConnection(com.handee.rpc.Connection) added}.
	 */
	 ...
	 * 
	 * @see #getRemoteObject(com.handee.rpc.Connection, int, Class...)
	 */
	...
	public void register(Map<Integer, Object> objects) {
		if (objects == null || objects.size() == 0) {
			return;
		}
		for (Map.Entry<Integer, Object> object : objects.entrySet()) {
			register(object.getKey().intValue(), object.getValue());
		}
	}
	 ...
	 * invoked on the update thread of the {@link com.handee.rpc.EndPoint} for
	 * this ObjectSpace and unless an
	 * {@link #setExecutor(java.util.concurrent.Executor) executor} has been
	 * set.
	 * 
	 * @param connection
	...
	protected void invoke(Connection connection, Object target, InvokeMethod invokeMethod) {
		if (DEBUG) {
			...
			debug("kryonet", connection + " received: " + target.getClass().getSimpleName() + "#" + invokeMethod.method.getName() + "(" + argString + ")");
		}
	...
	/**
	 * Identical to {@link #getRemoteObject(com.handee.rpc.Connection, int, Class...)} except
	 * returns the object cast to the specified interface type. The returned
	 * object still implements {@link com.handee.rpc.rmi.RemoteObject}.
	 */
	 ...
	 * remote end of the connection has not {@link #addConnection(com.handee.rpc.Connection)
	 * added} the connection to the ObjectSpace, the remote method invocations
	 * will be ignored.
	 * <p/>
	 * Methods that return a value will throw {@link com.handee.rpc.rmi.TimeoutException} if the
	 * response is not received with the
	 * {@link com.handee.rpc.rmi.RemoteObject#setResponseTimeout(int) response timeout}.
	 * <p/>
	 * If {@link com.handee.rpc.rmi.RemoteObject#setNonBlocking(boolean) non-blocking} is false
	 ...
	 * 
	 * @see com.handee.rpc.rmi.RemoteObject
	 */
	...
	static private class RemoteInvocationHandler implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Exception {
			...
			// The only time a invocation doesn't need a response is if it's
			// async
			...
			if (needsResponse) {
				byte responseID;
				synchronized (this) {
					// Increment the response counter and put it into the first
					// six bits of the responseID byte
					responseID = nextResponseNum++;
					if (nextResponseNum == 64)
						nextResponseNum = 1; // Keep number under 2^6, avoid 0
				...
				invokeMethod.responseID = 0; // A response info of 0 means to
												// not respond
			}
	...
	static public class InvokeMethod implements FrameworkMessage, KryoSerializable {
		...
		public Object[] args;
		// The top two bytes of the ID indicate if the remote invocation should
		// respond with return values and exceptions,
		// respectively. The rest is a six bit counter. This means up to 63
		// responses can be stored before undefined behavior
		// occurs due to possible duplicate IDs.
		public byte responseID;

		public void write(Kryo kryo, Output output) {
			output.writeInt(objectID, true);//4

			int methodClassID = kryo.getRegistration(method.getDeclaringClass()).getId();
			output.writeInt(methodClassID, true);//57 70
			...
			output.writeByte(responseID);//-63
		}
	...
	static class MethodComparator implements Comparator<Method> {
		...
		public int compare(Method o1, Method o2) {
			...
			if (!o1.getDeclaringClass().getName().equals(o2.getDeclaringClass().getName())){
				return -1;
			}
			throw new RuntimeException("Two methods with same signature [m1=" + o1.getDeclaringClass().getName() + "." + o1.getName() + ", m2=" + o2.getDeclaringClass().getName() + "." + o2.getName()
					+ "]!"); // Impossible.
		}
	}
	...
	static CachedMethod[] getMethods(Kryo kryo, Class<?> type) {
		...
		PriorityQueue<Method> methods = new PriorityQueue<>(Math.max(1, allMethods.size()), new MethodComparator());
	 ...
	 * 
	 * @see com.esotericsoftware.kryo.Kryo#register(Class,
